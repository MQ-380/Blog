...
export function* uploadArticleInfoFlow() {
  while (true) {
    ...
    if (res) {
      ...
      if (res.status) {
        yield put({
          type: actionTypes.PUBLISH_RESULT,
          content: '文章上传成功！',
          title: '上传文章成功',
          alertType: 'success'
        })
...
export function* editArticleFlow() {
  while (true) {
    ...
    if (res) {
      ...
      if (res.status) {
        yield put({
          type: actionTypes.EDIT_ARTICLE_RESULT,
          content: '文章修改成功！',
          title: '文章修改成功',
          alertType: 'success'
        })
...
export function* deleteArticleFlow() {
  while (true) {
    ...
    if (res) {
      ...
      if (res.status) {
        yield put({
          type: actionTypes.DELETE_ARTICLE_RESULT,
          content: '文章已被删除！',
          title: '文章已被删除',
          alertType: 'success'
        })
  ...
  while (true) {
    ...
    if (res) {
      ...
      if (res.status) {
        ...
        yield put({
          type: actionTypes.SET_ARTICLE_CONTENT,
          content: res.content,
...
export function* reviewCommentFlow() {
  while (true) {
    ...
    let res = yield call(reviewComment, {
    ...
    if (res) {
      ...
      if (res.status) {
        yield put({
          type: actionTypes.COMMENT_REVIEW_SUCCESS,
          comment: res.comment,
          articleId: res.articleId
        })
      } else {
        yield put({
          type: actionTypes.COMMENT_REVIEW_FAILED,
          content: '评论操作失败，请检查网络！',
          title: '',
          alertType: 'error'
        })
      }
