...
import { Button, Table, Tooltip, Menu, Icon, Dropdown, Modal } from 'antd'
...
class List extends Component {
  ...
  onSelectedChange = selectedRowKeys => {
    selectedRowKeys = selectedRowKeys.filter(
      item => this.props.user.filter(e => e._id === item).length !== 0
    )
  ...
  actions = (
    <Menu onClick={this.handleAction}>
      ...
      <Menu.Item key={'add'}>
        <span>
          <Icon type={'plus'}/>增加管理员
        </span>
      </Menu.Item>
      <Menu.Item key={'delete'}>
        <span>
          <Icon type={'minus'}/>删除所选用户
        </span>
      </Menu.Item>
    </Menu>
  );
    ...
    return (
      <div>
        ...
        <Modal
          ...
          onOk={() => {
            this.props.delete_item(this.state.selectedRowKeys)
          }}
