  ...
  handleSubmit = (type) => {
    this.refs.editor.validateFields((err, values) => {
      if (!err) {
        ...
        this.props.publish_article(values.title, content, type, this.refs.tags.state.tags, this.props.writer, values.linkName)
      }
    })
  }
  ...
  render () {
    return (
      <div>
        ...
        <p>
          <b>注意：若要根据编辑器内格式请使用文章发布，使用markdown发布字体、字号、间距等设定将失效。</b>
        </p>
