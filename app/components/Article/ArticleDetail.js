...
class ArticleDetail extends Component {
  ...
  isEdited = () =>
    (this.props.content_type === 'upload'
      ? false
      : this.refs.editor.editorInstance.getContent() !==
      this.props.article_content) ||
    this.refs.editorForm.validateFields((err, values) => {
      if (!err) {
        return (
          values.title !== this.state.nowArticle.articleName ||
          values.linkName !== this.state.nowArticle.linkName
        )
      }
    }) ||
    ...
    this.refs.editorForm.validateFields((err, values) => {
      if (err) {
        ...
        if (this.props.content_type === 'upload') {
          this.props.edit_upload_info(
            values.title,
            this.state.newFileName,
            this.state.nowArticle.fileName,
            this.refs.tags.state.tags,
            values.linkName,
            this.state.nowArticle.writer,
            this.state.nowArticle._id
          )
        } else {
          this.props.edit_article(
            ...
            values.title,
            values.linkName,
            this.refs.editor.editorInstance.getHTMLContent(),
            this.refs.tags.state.tags
          )
        }
      }
      ...
      <div>
        <div style={{position: 'relative'}}>
          ...
          <div style={{position: 'absolute', right: '5%'}}>
            <Button onClick={this.editArticle}>保存修改</Button>
        ...
        {/html|md/.test(this.props.content_type) && (
          <div>
            <Editor
              ref={'editor'}
              contentFormat={'html'}
  ...
  componentDidMount () {
    let nowArticle = this.props.article_list.filter(
...
const mapDispatchToProps = dispatch => {
  return {
    get_article_content: bindActionCreators(
      publishAction.get_article_content,
      dispatch
    ),
    ...
    clear_article_info: bindActionCreators(
      publishAction.clear_article_info,
      dispatch
    ),
    edit_article: bindActionCreators(publishAction.edit_article, dispatch),
    clear_result_msg: bindActionCreators(
      publishAction.clear_result_msg,
      dispatch
    ),
    edit_upload_info: bindActionCreators(
      publishAction.edit_upload_info,
      dispatch
    )
  };
};
