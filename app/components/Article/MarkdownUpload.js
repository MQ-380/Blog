...
import { Form, Input, Upload, Button, Icon, message, Modal } from 'antd/lib/index'
  ...
  render() {
    return (
      <div>
        ...
        <UploadForm {...this.props} onChange={(changedFields)=>
        {if(changedFields.linkNameEdit) this.props.set_link_name(changedFields.linkNameEdit);
         else this.props.set_article_name(changedFields.articleNameEdit)}}/>

        <Tags  ref={'tags'}/>

        <Button onClick={()=>{this.props.upload(this.props.name,this.props.linkNameEdit.value,this.props.articleNameEdit.value,this.refs.tags.state.tags,this.props.writer)}}>
          确认发表
        </Button>
        <Button onClick={()=>{
          Modal.confirm({
